...
-- We'll start the file with BEGIN and end the file with COMMIT. This is like a try block in JavaScript. 
-- When PostgreSQL runs a command such as CREATE or INSERT, it's called a transaction. PostgreSQL is 
-- adding BEGIN before each command and COMMIT after each for us. The "begin" and "commit" commands 
-- are specifying the start and end of a transaction. We explicitly set the "begin" and "commit" 
-- around multiple commands to combine them into one transaction. This means that if the first 
-- one works but the second one fails, Postgres will undo the first command so we're not left 
-- with a "half-cooked" state. This is great for seeding or altering multiple tables.